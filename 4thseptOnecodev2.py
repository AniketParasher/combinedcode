...
def create_download_link(file_data, file_name, link_text):
    b64 = base64.b64encode(file_data).decode()  # Encode to base64
    return f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{file_name}" class="download-link"><img src="https://img.icons8.com/material-outlined/24/000000/download.png" class="download-icon"/> {link_text}</a>'
    ...
    # if st.session_state['mapped_data'] is not None:
        ...
        if st.button("Click to Generate PDFs and Zip"):
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                # Create the custom download link for the zip file
                zip_download_link = create_download_link(zip_buffer.getvalue(), "attendance_Sheets.zip", "Click to Download Zip File")
